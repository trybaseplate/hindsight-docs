...
Tool calls share authorization, scope and role checks, rate limits, and usage accounting with their MCP equivalents. See [API keys](/integrations/api#api-keys) and [Rate limits](/integrations/api#rate-limits) for details.
...
Common status codes are `400` for invalid input, `401` for an invalid API key, `403` for insufficient scope or a role without access to the tool, and `429` for a rate limit or applicable usage quota. For exact request and response examples of the deal lookup flow, see [Select Deal](/api-reference/select-deal) and [Get Deal](/api-reference/get-deal).
