...
Rate limits: Organization-wide and per-API-key limits apply. Exports may consume a separate usage quota.
```
</Accordion>
...
Exports are subject to the organization-wide and per-API-key request limits. Export operations may also consume a separate usage quota.

When a request limit is exceeded, the API returns `429 Too Many Requests`. Handle it using `X-RateLimit-Limit`, `X-RateLimit-Remaining`, and `X-RateLimit-Reset`; these public headers represent the active response limit.
