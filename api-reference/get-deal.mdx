...
Organization-wide and per-API-key limits apply. See [Rate limits](/integrations/api#rate-limits) for returned headers and retry guidance. A request with no IDs or more than ten IDs returns `400`.
