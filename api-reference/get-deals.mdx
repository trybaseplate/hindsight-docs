...
[
  {
    ...
    "summary": "Executive summary text for the deal…",
...
**Example:**
```
GET /deals?columns=name,amount,status_id,close_date,owner_id
...
The relational columns `drivers`, `competitors`, and `features` return flattened arrays (see [Drivers, competitors, and features](#response-format) below) and are included with `*`. `summary` returns only the deal's executive-summary string, not the full internal deal-story object.
</Note>
...
`summary` returns only the deal's **executive summary** string (extracted from the internal deal-story JSON). It is `null` for deals that haven't been analyzed yet.
