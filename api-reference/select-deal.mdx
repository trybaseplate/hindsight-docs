...
Organization-wide and per-API-key limits apply. See [Rate limits](/integrations/api#rate-limits) for returned headers and retry guidance. Invalid input returns `400`; unauthorized or disallowed calls return `401` or `403`.
