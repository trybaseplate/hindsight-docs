...
<Tabs>
  ...
  <Tab title="Claude Desktop">
    <Note>
      The one-click `hindsight.mcpb` bundle install is deprecated. Connect via HTTP instead by adding Hindsight directly to your Claude Desktop config file:
    </Note>
    ...
    ```json
    {
      "mcpServers": {
        "hindsight": {
          "type": "http",
          "url": "https://app.usehindsight.com/api/mcp",
          "headers": {
    ...
    Restart Claude Desktop after saving. Generate an API key from **Settings → API Keys** in the Hindsight app.
...
<Tabs>
  <Tab title="Claude Web / Desktop">
    ...
    **2. Add it again**
    ...
    Click **+ → Add custom connector**, set the URL to `https://app.usehindsight.com/api/mcp`, and click **Add**.
...
| `run_sandbox_dataset_analysis` | Run a user-supplied Python script against a dataset from `create_dataset` in an isolated, network-disabled sandbox, printing the requested aggregate or table and optionally saving up to 3 charts. Replaces the retired `analyze_dealgrid_subset`. |

**Typical flow:** `pull_schema` → `create_dataset` → `run_sandbox_dataset_analysis`.
