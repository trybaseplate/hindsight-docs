...
| `search_comparable_deals` | Given a source deal, return the org's configured set of historical comparable deals — the same algorithm behind the in-app comparables feature, run on demand. Retrieval only; typically followed by `get_deal` / `search_deal_documents` on the returned IDs. |
