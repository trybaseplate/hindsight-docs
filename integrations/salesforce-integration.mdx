...
{/*
  IP-Bound Token Refresh — choose the accurate wording before publishing:
  - If static egress + Salesforce IP restrictions are LIVE in production: set Status to "Enforced"
    and reword to "...are bound to an approved static IP range."
  - If not yet provisioned: keep "Available" as written (true: the architecture supports it,
    it is not yet enforced). Do NOT claim "Enforced" until it is.
*/}
