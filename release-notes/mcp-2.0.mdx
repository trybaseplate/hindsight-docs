...
- **Workspace selection** — fixed cases where picking your organization during sign-in wouldn't complete, including inside popup windows.
- **Longer-lived sessions** — the server now issues refresh tokens, so you stay signed in without needing to reconnect as often.
- **Clearer tool errors** — tools now report readable names and descriptive messages when a call fails, instead of a generic error.
