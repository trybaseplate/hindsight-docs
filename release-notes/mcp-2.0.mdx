...
- **Stay signed in longer.** OAuth connections now support refresh tokens, so long-running sessions renew automatically instead of forcing you to reconnect.
...
## Per-workspace tool access & clearer tool metadata

<New category="Admin" />

Admins can now choose which MCP tools are enabled for their workspace, so you can scope exactly what an assistant is allowed to call. Tools also carry readable titles and behavior hints (e.g. read-only vs. state-changing) instead of raw function names, and failed tool calls now return a clear error message instead of failing silently.
