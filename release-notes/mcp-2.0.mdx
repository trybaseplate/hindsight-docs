...
## Tool access controls & longer sessions

<New category="Developer Tools" />

Follow-up improvements based on early feedback from the 2.0 launch.

- **Admin tool access controls** — Admins can now choose which Hindsight tools are exposed through MCP for their organization, instead of exposing the full set to everyone.
- **Friendlier tools** — Tools show clearer names and descriptions in your assistant's tool picker, and error messages are more helpful when a call doesn't go through.
- **Stay signed in longer** — MCP connections now refresh automatically in the background, so you won't be prompted to reauthenticate as often.
